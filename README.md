...
The UI is deliberately split so a large ingest remains responsive while its outcome is easy to scan. The console now leads with a run-level health summary (records processed, recognized formats, newly learned formats, attention-needed records, and average latency), followed by compact result cards. Each card puts the normalized message and useful fields first, translates internal processing modes into plain language, and links to the full record detail. Large JSON payloads and search live in the explorer rather than crowding the primary results view.
...
- **Record explorer storage**: captured runs live in the browser (newest 3, `localStorage`). Clearing site data or switching browser/profile loses them; the console's result cards reflect the current session.
