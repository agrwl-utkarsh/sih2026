...
def test_49_clear_endpoint_empties_every_cache():
    """Ingest real logs first so all four caches hold something."""
    client.post("/api/logs/ingest", json={"logs": [
    ...
    assert parser.family_cache, "expected a family to be learned"
    assert parser.cache, "expected a fingerprint rule to be cached"
    ...
    assert "templates" in body["cleared"]
    assert "template_rules" in body["cleared"]
    assert "quarantine" in body["cleared"]

    # Ground truth: everything is actually empty now.
    assert parser.family_cache == {}
    assert parser.cache == {}
    assert parser._parsed_fps == {}
    assert template_tier.quarantine_view()["items"] == []
    assert all(v == 0 for v in template_tier.stats.values())


def test_50_clear_makes_next_ingest_discover_again():
    """The point of the button: a known family must re-enter Discovery."""
    ...
    first = client.post("/api/logs/ingest", json={"logs": [csv_a]})
    assert first.json()["processed_logs"][0]["mode"] == "Discovery"
    second = client.post("/api/logs/ingest", json={"logs": [csv_b]})
    assert second.json()["processed_logs"][0]["mode"] == "Cached"

    client.post("/api/logs/clear")

    again = client.post("/api/logs/ingest", json={"logs": [csv_b]})
    assert again.json()["processed_logs"][0]["mode"] == "Discovery"


def test_51_clear_is_idempotent_and_reports_zeroes():
